...
mainDir <- "Z:"
inDir <- file.path("Z:", "QUBS_LAS", "LAS")
outDir<- file.path("Z:", "QUBS_LAS", "Derived")
...
name <- 'processed1-5meter'# strsplit(image, '[.]')[[1]][1]
...
system(paste(file.path(fusionPath, "canopymodel.exe"),
       p("/ground:",file.path(outDir, p(name, "-final", ".dtm"))),
       file.path(outDir, p(name, "-surface", ".dtm")),
       "5 M M 1 12 2 2",
       allImageString,
       sep=" "))
